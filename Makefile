...
XMR_K3S_NAME=$(USERNAME)/$(XMR_BASIC_REPO):$(TAG)
...
xmr-multiarch:
	docker buildx build -t $(XMR_NAME) --platform linux/amd64,linux/arm64 -f ./xmr/Dockerfile.multiarch --push ./xmr

cpp-multiarch-arm:
	docker buildx build -t $(CPP_NAME) --platform linux/arm64 -f ./cs104/Dockerfile --load ./cs104

xmr-k3s:
	docker buildx build -t $(XMR_K3S_NAME) --platform linux/amd64,linux/arm64 -f ./xmr/Dockerfile.multiarch-k3s --push ./xmr

xmr-k3s-ld-x86:
	docker buildx build -t $(XMR_K3S_NAME) --platform linux/amd64 -f ./xmr/Dockerfile.multiarch-k3s --load ./xmr

xmr-k3s-ld-arm:
	docker buildx build -t $(XMR_K3S_NAME) --platform linux/arm64 -f ./xmr/Dockerfile.multiarch-k3s --load ./xmr
